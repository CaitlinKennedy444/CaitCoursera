...
na <- apply(training1, 2, function(x) {sum(is.na(x))})
train_clean1 <- training1[na == 0]
test_clean1 <- testing1[na == 0]

#Remove columns with missing data
empty_data <- apply(train_clean1, 2, function(x) {sum(x == "")})
train_clean2 <- train_clean1[empty_data == 0]
test_clean2 <- test_clean1[empty_data == 0]

# Remove the columns that are not numeric and will not be contributing
training <- train_clean2[-c(1:7)]
testing <- test_clean2[-c(1:7)]
...
Fit1 <- train(classe ~ . , method = "rpart", data = myTrain)
pred1 <- predict(Fit1, myTest)
cm1 <- confusionMatrix(table(pred1, myTest$classe))
cm1
#The accuracy of this model fit is 49%

Fit2 <- train(classe ~ . , method = "rf", data = myTrain, prox=TRUE)
pred2 <- predict(Fit2, myTest)
cm2 <- confusionMatrix(table(pred2, myTest$classe))
cm2
#The accuracy of thie model fit is 99%

Fit3 <- train(classe ~ . , method = "gbm", data = myTrain, verbose=FALSE)
pred3 <- predict(Fit3, myTest)
cm3 <-confusionMatrix(table(pred3, myTest$classe))
cm3
...
myTrain <- training[inTrain, ]
myTest <- training[-inTrain, ]
model_stacked <- data.frame(pred1, pred2, pred3, classe=myTest$classe)
str(model_stacked)
fit_stacked <- train(classe~., method="rf", data=model_stacked)
...
cm1.1 <- confusionMatrix(pred1, myTest$classe)
cm2.1 <- confusionMatrix(pred2, myTest$classe)
cm3.1 <- confusionMatrix(pred3, myTest$classe)
cm4.1 <- confusionMatrix(predict_stacked_test, myTest$classe)
...
print(paste(cm1.1$overall[1], cm2.1$overall[1], cm3.1$overall[1], cm4.1$overall[1]))
#[1] "0.490909090909091 0.994222599830076 0.960577740016992 0.994222599830076"
#Accuracies of the stacked model fit should predict better or at least as good as than the accuracies of the individual methods as described above
#############################################################################################################
...
predictionResult <- predict(Fit2, testing)
